...
同时集成了本地便签功能，可快速新建浮动便签，并通过管理窗口搜索、打开和删除笔记。

项目按系统拆分：

- `transTags_windows`：Windows 版本，使用 Win32 API 实现。
- `transTags_linux`：Ubuntu/Linux 版本，主版本使用 C++/Qt Widgets、QtSql + X11/XFixes/EWMH 实现。

## 演示截图

![transTags 透明窗口与鼠标穿透演示](docs/assets/transTags_demo.png)
...
- 将窗口移动到屏幕中央。
- 每次操作后在右下角显示短提示。
- 新建浮动便签，自动保存标题、正文、位置、大小和置顶状态。
- 在便签管理窗口中按标题或正文关键词查询笔记。
...
- Ubuntu/Linux 版用 Qt Widgets 实现托盘、设置窗口和便签界面，底层通过 X11/XFixes/EWMH 修改窗口透明度、输入区域、活动窗口和置顶状态。完整窗口控制功能需要在 `Ubuntu on Xorg` 会话下运行。
- 便签是 transTags 自己创建的普通窗口，数据保存在本地 `transTags_notes.sqlite` 数据库中；Windows 版链接 SQLite 单文件源码，Linux 版使用 QtSql 的 SQLite 驱动。
...
- 右键托盘图标，选择 `新建便签` 或 `管理便签`。
- 便签窗口支持保存、置顶和删除；关闭前会自动保存。
- 便签数据保存在本地 `transTags_notes.sqlite` 中，使用 SQLite 存储。
- Windows 版可用 `config.ini` 中的 `NewNoteModifiers`、`NewNoteKey`、`EnableNotes` 调整新建便签快捷键和开关。
- Ubuntu/Linux 版可在设置窗口启用或关闭便签，默认使用 `Alt + N` 新建便签。
